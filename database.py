...
def init_db():
    ...
    # Таблица для хранения OAuth-токенов DonationAlerts (для VK)
    c.execute('''
...
# ----- Работа с балансом и моделью пользователя -----
...
def set_current_model(user_id, model):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("UPDATE users SET current_model = ? WHERE user_id = ?", (model, user_id))
    if c.rowcount == 0:
        c.execute("INSERT INTO users (user_id, current_model) VALUES (?, ?)", (user_id, model))
    conn.commit()
    conn.close()
...
def save_generation(user_id, model, prompt):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("INSERT INTO generations (user_id, model, prompt) VALUES (?, ?, ?)",
...
# ----- Работа с токенами DonationAlerts (для VK-бота) -----
def save_donation_token(access_token, refresh_token, expires_in):
    ...
    conn.commit()
    conn.close()
    logger.info("Токен DonationAlerts сохранён")

...
def build_payload(model: str, prompt: str = None, image_url: str = None) -> dict:
    ...
    payloads = {
        ...
        "gpt-image-2": {"prompt": prompt, "size": "1024x1024", "n": 1, "response_format": "url"},
...
# ------------------- ОБРАБОТЧИК ИЗМЕНЕНИЯ РАЗМЕРА (GPT Image 2) -------------------
...
async def handle_resize_size(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Формируем payload для gpt-image-2
    payload = {
...
# ------------------- УПАКОВКА ГРУППЫ ВК (полная версия) -------------------

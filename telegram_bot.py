...
async def handle_deepseek_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if action == 'prompt_image':
        system_prompt = (
            "Ты — профессиональный генератор промтов для создания изображений. Твоя задача — превратить краткий запрос пользователя в развернутый, "
            "максимально детализированный и структурированный промт на РУССКОМ языке. Следуй строго предложенному формату, чтобы промт было легко "
            "копировать и использовать в любых нейросетях (Midjourney, DALL-E, Kandinsky и др.).\n\n"
            "**ФОРМАТ ПРОМТА:**\n"
            "1. **Субъект:** подробное описание главного объекта, его позы, действий, эмоций, внешности.\n"
            "2. **Действие:** что именно происходит в кадре (динамичное или статичное).\n"
            "3. **Окружение:** место действия, окружение, фон, атмосфера.\n"
            "4. **Стиль:** художественный стиль (например, фотореализм, киберпанк, масляная живопись, аниме).\n"
            "5. **Освещение и цветовая палитра:** тип освещения (мягкий рассеянный свет, неон, контражур), основные цвета и их сочетания.\n"
            "6. **Технические детали и атмосфера:** детали (погода, время суток, настроение, эффекты), технические параметры (8K, высокое разрешение).\n\n"
            "**Ограничения (что НЕЛЬЗЯ делать):**\n"
            "- Не добавляй лишних объяснений и своих комментариев (слов: „Вот твой промт“, „Промт для изображения“ и т.п.).\n"
            "- Не используй разметку Markdown внутри промта (жирный шрифт, курсив, списки). Разрешается только обычный текст с переносами строк.\n"
            "- Не уменьшай количество деталей. Промт должен быть очень подробным (минимум 50–150 слов).\n\n"
            "**ОБЯЗАТЕЛЬНО используй в промте сильную визуальную метафору, которая делает описание ёмким и образным (например, „солнце плавится как янтарь“).**\n\n"
            "От этого зависит качество будущей картинки. Пиши чётко, по делу, максимально красочно и визуально."
        )
        user_prompt = f"{system_prompt}\n\nЗАПРОС ПОЛЬЗОВАТЕЛЯ ДЛЯ ГЕНЕРАЦИИ ПРОМТА: «{user_input}»\n\nСОЗДАЙ ПРОМТ, СЛЕДУЯ УКАЗАННОМУ ФОРМАТУ (ТОЛЬКО ПРОМТ, БЕЗ ЛИШНИХ СЛОВ):"
    elif action == 'prompt_video':
        system_prompt = (
            "Ты — профессиональный генератор промтов для создания ВИДЕО. Твоя задача — превратить краткий запрос пользователя в развернутый, "
            "максимально детализированный и структурированный промт на РУССКОМ языке. Следуй строго предложенному формату, чтобы промт было легко "
            "копировать и использовать в любых видеогенераторах (Kling, Runway Gen-2, Pika Labs и др.).\n\n"
            "**ФОРМАТ ПРОМТА:**\n"
            "1. **Субъект:** подробное описание объекта(ов), их внешность, одежда, эмоции.\n"
            "2. **Движение и действие:** как двигается субъект, что происходит в кадре, динамика происходящего.\n"
            "3. **Движение камеры:** тип движения камеры (наезд, отъезд, панорама, кружение, steadicam).\n"
            "4. **Окружение и атмосфера:** место действия, фон, погода, время суток, освещение.\n"
            "5. **Стиль:** художественный стиль (кино, аниме, футуризм, Ghibli).\n"
            "6. **Цветовая палитра и свет:** основные цвета, тип освещения (неон, мягкий, жёсткий).\n"
            "7. **Настроение и технические детали:** общая атмосфера (мистическая, радостная, тревожная), 4K, высокое разрешение.\n\n"
            "**Ограничения (что НЕЛЬЗЯ делать):**\n"
            "- Не добавляй лишних объяснений и своих комментариев (слов: „Вот твой промт“, „Промт для видео“ и т.п.).\n"
            "- Не используй разметку Markdown внутри промта (жирный шрифт, курсив, списки). Разрешается только обычный текст с переносами строк.\n"
            "- Не уменьшай количество деталей. Промт должен быть очень подробным (минимум 50–150 слов).\n\n"
            "**ОБЯЗАТЕЛЬНО используй в промте сильную визуальную метафору, которая делает описание ёмким и образным (например, „небо затягивает в воронку из облаков“).**\n\n"
            "От этого зависит качество будущего видео. Пиши чётко, по делу, максимально красочно и визуально."
        )
        user_prompt = f"{system_prompt}\n\nЗАПРОС ПОЛЬЗОВАТЕЛЯ ДЛЯ ГЕНЕРАЦИИ ПРОМТА: «{user_input}»\n\nСОЗДАЙ ПРОМТ, СЛЕДУЯ УКАЗАННОМУ ФОРМАТУ (ТОЛЬКО ПРОМТ, БЕЗ ЛИШНИХ СЛОВ):"
    else:
    ...
    try:
        ...
        if answer:
            # Очищаем ответ от возможных маркдаунов и лишних "обёрток"
            answer = answer.strip()
            # Если модель добавила пояснения — удаляем их
            for prefix in ["ПРОМТ:", "ВОТ ПРОМТ:", "ОТВЕТ:", "ПРОМТ ДЛЯ ИЗОБРАЖЕНИЯ:", "ПРОМТ ДЛЯ ВИДЕО:"]:
                if answer.startswith(prefix):
                    answer = answer[len(prefix):].strip()
            # Сжимаем множественные переносы строк, но сохраняем структуру
            answer = '\n'.join(line.strip() for line in answer.split('\n') if line.strip())
            # Если промт получился слишком коротким, логгируем
            if len(answer) < 30:
                logger.warning(f"Сгенерирован слишком короткий промт: {answer[:100]}")
    except Exception as e:
    ...
    if answer:
        # Отправляем промт в моноширинном блоке для удобного копирования
        await update.message.reply_text(
            f"✨ **ВАШ ПРОМТ ГОТОВ:**\n\n"
            f"```\n{answer}\n```",
            parse_mode="Markdown"
        ...
        save_message(user_id, "assistant", answer)
        await update.message.reply_text(
            "📋 **НАЖМИТЕ НА БЛОК ВЫШЕ, ВЫДЕЛИТЕ ЕГО И СКОПИРУЙТЕ.**\n\n"
            "ЗАТЕМ ВСТАВЬТЕ ЭТОТ ПРОМТ В ЛЮБУЮ НЕЙРОСЕТЬ ДЛЯ ГЕНЕРАЦИИ.",
            reply_markup=get_popular_menu_keyboard()
        )
    else:

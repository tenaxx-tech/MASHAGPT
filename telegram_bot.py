...
PAID_IMAGE_PRICE = 2
ADMIN_IDS = [466829859]   # замените на свой ID
...
AWAIT_RESIZE_IMAGE, AWAIT_RESIZE_SIZE = 40, 41
...
def get_popular_menu_keyboard():
    keyboard = [
        ...
        [KeyboardButton("🖼 9. Изменить размер изображения")],
    ...
    if model == "gpt-image-2":
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=payload, headers=headers, timeout=aiohttp.ClientTimeout(total=60)) as resp:
                if resp.status != 200:
                    error_body = await resp.text()
                    raise Exception(f"GPT Image 2 error {resp.status}: {error_body}")
                data = await resp.json()
                ...
                async with session.get(media_url) as img_resp:
                    if img_resp.status != 200:
                        raise Exception(f"Ошибка скачивания: {img_resp.status}")
...
def build_payload(model: str, prompt: str = None, image_url: str = None) -> dict:
    ...
    payloads = {
        ...
        "gpt-image-1-5-text-to-image": {"prompt": prompt, "quality": "medium"},
    ...
    for model_id, label, price in video_models:
        ...
        if text.strip() == btn_text.strip():
            ...
            if input_type == ("video", "image"):
                await update.message.reply_text("🔹 Анимация персонажа\n\n1️⃣ Отправьте видео-референс\n2️⃣ Затем изображение персонажа\nОтправьте первое видео:", reply_markup=get_cancel_keyboard())
                return AWAIT_VIDEO_FOR_ANIMATE
            elif input_type == ("image", "text"):
                await update.message.reply_text("🔹 Image-to-Video\n\n1️⃣ Отправьте изображение\n2️⃣ Затем текстовое описание\nОтправьте первое фото:", reply_markup=get_cancel_keyboard())
    ...
    if category == "image" and price == 0:
        ...
        if used >= 5:
            ...
            if balance >= PAID_IMAGE_PRICE:
                ...
                await update.message.reply_text(f"❌ Лимит исчерпан. Нужно {PAID_IMAGE_PRICE} промтов.", reply_markup=get_main_keyboard())
                return MAIN_MENU
    elif price > 0:
...
async def handle_popular_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if text == "🔙 Главное меню":
        ...
        context.user_data['model_price'] = 0
        context.user_data['media_category'] = 'image'
        await update.message.reply_text("Отправьте изображение для улучшения:", reply_markup=get_cancel_keyboard())
        return AWAIT_IMAGE_ONLY
...
# ------------------- НОВЫЙ ОБРАБОТЧИК ИЗМЕНЕНИЯ РАЗМЕРА (GPT Image 2) -------------------
...
async def handle_resize_size(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    payload = {"model": "gpt-image-2", "prompt": f"Измени размер этого изображения ровно до {size}. Сохрани содержимое.", "n": 1, "size": size, "response_format": "url"}
    try:
        result_bytes, media_url = await masha_media_generate("gpt-image-2", payload)
        if result_bytes:
            compressed = await compress_image(result_bytes)
...
# ------------------- ОСТАЛЬНЫЕ ОБРАБОТЧИКИ (полностью) -------------------
async def handle_deepseek_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if action == 'prompt_image':
        sys_prompt = "Ты эксперт по промтам. Создай JSON (classic и hightech) по запросу. Только JSON."
        prompt = f"{sys_prompt}\nЗапрос: {user_input}\nJSON:"
    elif action == 'prompt_video':
        sys_prompt = "Ты эксперт по видеопромтам. Создай JSON (classic и hightech). Только JSON."
        prompt = f"{sys_prompt}\nЗапрос: {user_input}\nJSON:"
    else:
    ...
    action_task = asyncio.create_task(send_action_loop(update, ChatAction.TYPING, stop_action))
    try:
        answer = await masha_text_generate(prompt, history, "deepseek-chat")
        answer = answer.strip()
...
async def handle_animate_photo_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text("Теперь отправьте описание движения (или 'пропустить'):", reply_markup=get_cancel_keyboard())
...
async def handle_edit_image(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text("Теперь отправьте описание изменений:", reply_markup=get_cancel_keyboard())
...
async def handle_edit_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if result_bytes:
        ...
        await update.message.reply_text(f"📥 Оригинал: {media_url}")
        if not paid: increment_weekly_image_count(user_id)
        save_message(user_id, "user", f"edit: {prompt_text}")
...
async def handle_avatar_audio(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if result_bytes:
        ...
        await update.message.reply_text(f"📥 Оригинал: {media_url}")
        save_message(user_id, "user", "avatar")
...
async def handle_animate_image(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if result_bytes:
        ...
        await update.message.reply_text(f"📥 Оригинал: {media_url}")
        save_message(user_id, "user", "animate")
...
async def vk_package_colors(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text("✅ Параметры сохранены!\nВыберите элемент упаковки:", reply_markup=get_vk_package_element_keyboard())
...
async def vk_package_element_select(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if text == "🖥 Обложка ПК (1920×768)":
        ...
        await update.message.reply_text("Выберите пункт из меню.", reply_markup=get_vk_package_element_keyboard())
        return VK_PACKAGE_ELEMENT_SELECT
...
async def generate_final_image(update, context, message):
    ...
    if backgrounds:
        ...
        if not bg_bytes:
            await message.reply_text("❌ Ошибка: фон не найден.")
            return
        img_with_text = add_text_to_image(bg_bytes, text_data, target_size=(width, height))
        compressed = await compress_image(img_with_text)
